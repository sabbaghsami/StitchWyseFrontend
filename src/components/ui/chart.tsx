...
function sanitizeCssValue(value: string): string | null {
  ...
  if (!trimmed || trimmed.length > 120 || UNSAFE_CSS_CHARS.test(trimmed)) {
    return null;
  }
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const scopedChartId = sanitizeCssIdentifier(id);
    ...
    .map(([theme, prefix]) => {
      const declarations = colorConfig
        .map(([key, itemConfig]) => {
          ...
          return `  --color-${sanitizeCssIdentifier(key)}: ${color};`;
        })
      ...
      return `${prefix} [data-chart="${scopedChartId}"] {\n${declarations.join("\n")}\n}`;
    })
